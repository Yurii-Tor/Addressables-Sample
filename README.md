...
<!--
  After the first successful "WebGL demo" workflow run, replace this comment with:

  **▶ [Play in your browser](https://yurii-tor.github.io/Addressables-Sample/)**

  ![Gameplay](Docs/media/demo.gif)

  Record the GIF at 800x450 covering: startup -> tap -> score increment ->
  "Loading image..." -> new texture -> deliberate miss -> red flash.
  The on-screen diagnostics overlay should be visible throughout.
-->
...
| **Deterministic project generation** | The scene, prefab, material, fallback texture, post-processing profile, and Addressables groups are all generated by idempotent Editor tooling and asserted by a structural validator that runs in CI. |
| **Real remote delivery** | Round-texture bundles are packed separately and served over HTTPS from Cloudflare Pages, so each round is an independent network request. |
...
Everything under `Assets/_Project/Scenes`, `Prefabs`, `Materials`, `Textures`, and
